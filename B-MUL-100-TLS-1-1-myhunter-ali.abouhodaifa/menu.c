...
void menu(game_t game)
{
    ...
    while (loop && sfRenderWindow_isOpen(game.w)) {
        ...
        sfMouse_getPositionRenderWindow(game.w).x > 500 &&
        sfMouse_getPositionRenderWindow(game.w).x < 988 &&
        sfMouse_getPositionRenderWindow(game.w).y > 500 &&
...
game_t recreate_game(game_t game)
{
    ...
    game.text = sfTexture_createFromFile("resource/picture/55.png", NULL);
    ...
    sfSprite_setTexture(game.cursorSpr, sfTexture_createFromFile(\
        "resource/picture/66.png", NULL), sfTrue);
    return (game);
}
